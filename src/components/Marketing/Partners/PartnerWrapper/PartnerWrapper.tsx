...
// types
import { IPartner } from "types/marketing";
...
const PartnerWrapper: React.FC<PartnerWrapperProps> = ({
  children,
  partner,
  moveItem,
}) => {
  ...
  const [{ highlighted, hovered }, drop] = useDrop({
    ...
    drop(item: { order: number }) {
      ...
      const hoverOrder = partner.order;
    ...
    hover(item: { order: number }, monitor) {
      ...
      const hoverIndex = partner.order;
