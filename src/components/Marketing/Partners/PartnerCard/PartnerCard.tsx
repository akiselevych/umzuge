...
import { deletePartner, updatePartner } from "reduxFolder/slices/marketingPartners.slice";
//components
import ImageUploader from "components/ImageUploader/ImageUploader";
//type
import { IPhotoReview } from "types/marketing";
...
interface Props extends IPhotoReview {
  imagesLength: number;
}

const PartnerCard: FC<Props> = ({ id, image, imagesLength, order }) => {
  ...
  const onDelete = () => {
    ...
    dispatch(deletePartner(id)).then(() => {
      setDeleteBtnValue("Delete");
    });
  };

  const deleteBtn =
    deleteEmployeesLoadingStatus === "idle" && deleteBtnValue === "Delete"
      ? "Delete"
      : "loading...";
