...
interface PhotoReviewWrapperProps {
  children: React.ReactNode;
  photoReview: IPhotoReview;
  moveItem: (dragIndex: number, hoverIndex: number) => void;
}

const PhotoReviewWrapper: React.FC<PhotoReviewWrapperProps> = ({
  children,
  photoReview,
  moveItem,
}) => {
  ...
  const [{ highlighted, hovered }, drop] = useDrop({
    ...
    drop(item: { order: number }) {
      ...
      const hoverOrder = photoReview.order;
    ...
    hover(item: { order: number }, monitor) {
      ...
      const hoverIndex = photoReview.order;
